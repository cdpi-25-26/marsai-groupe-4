...
async function getTop50(req,res){
    try{
        const top50 = await Upload.findAll({
      ...
      limit: 50,
        ...
        if (top50.length === 0){
            ...
            await Upload.update({ phase_status: 'phase2' }, { where: { id:{[Op.in]: ids} } }); //L'OP.in en JavaScript vérifie si une propriété spécifique existe dans un objet ou dans sa chaîne de prototypes.
        res.json(top50);
    } catch (error) {
